...
#'                      If there aren't any names, the rows and columns should present the same order as in the cell groups (given in S4Vectors::metadata(data_obj)[["seismicGWAS.data"]][["group_info"]])
...
cal_sscore = function(data_obj, out_group_mat = NULL){
  ...
  if (seismic_slot_is_null(data_obj, "group_info")){
    ...
    if(seismic_slot_is_null(data_obj, "original_group_info")){ #use the original one 
      group_info = get_seismic_slot(data_obj,"group_info")
    }else{
      group_info = get_seismic_slot(data_obj,"original_group_info")
    }
  }
  if (any(!c("cell_num","mean_mat","var_mat","ratio_mat") %in% names(group_info))){
    stop("Please run cal_stat() first")
  }
  ...
  cell_num = group_info[["cell_num"]]
  mean_mat = group_info[["mean_mat"]]
  var_mat = group_info[["var_mat"]]
  ratio_mat = group_info[["ratio_mat"]]
  ...
  data_obj = group_info %>%
    ...
    purrr::set_names(c(names(group_info), "sscore")) %>%
