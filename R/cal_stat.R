...
#' @param meta_data A cell (column) metadata dataframe. Rownames of the data frame should be the same as the colnames of the expression matrix (keep cell ID order correct).
...
#' @export
#' 
cal_stat = function(data_obj,  group, meta_data = NULL, assay_name = "logcounts", filter_rare_ct = TRUE, filter_thres = 20, mean_only=FALSE) {
  #check input type
  ...
  if(any(is.na(ident_group))){
    ...
    message("The groupping column in the cell metadata provided seems to include NA ones, which have been replaced with strings instead.")
  }
