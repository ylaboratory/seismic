#' This function maps feature (gene) names to either entrez or other format, similar to `trans_mmu_to_hsa_stat`.
...
#' @param mapping_df A data frame to specify the mapping. The first column should contain all the featyre names while the second column indicates the new feature names to be mapped to.
...
munge_sce_mat = function(data_obj,  mapping_df, assay_type = "all",multi_mapping = "sum") {
  ...
  for (assay_name in assay_type){
    ...
    single_mat = single_mat %>% magrittr::set_rownames(all_mapping %>% filter(! .[[1]] %in% multiple_mapping[[1]] ) %>% pull(1))
  ...
  out_data = SingleCellExperiment::SingleCellExperiment(
    ...
    rowData = data.frame(rownames(new_assay[[1]]), row.names = rownames(new_assay[[1]]),fix.empty.names = FALSE) %>% magrittr::set_colnames(colnames(mapping_df)[2])
  )
