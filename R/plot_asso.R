...
plot_asso = function(data_obj, trait,show_value = "FDR", plot_top_option = NULL ,group = NULL, significance = TRUE  ) {
  ...
  if(is.null(metadata(data_obj)[["association"]][[trait]])){
    stop("The trait-cell type association does not exist")
  }
  if( !is.null(plot_top_option) & !all(all.vars(rlang::parse_expr(plot_top_option)) %in% c("cell_type","Pvalue","FDR","rank",colnames(metadata(data_obj)[["cell_type_anno"]])))){
    stop("Something's wrong with the plot_top_option. Not all columns exist. ")
  }
  asso_df = metadata(data_obj)[["association"]][[trait]] %>% 
    dplyr::arrange(Pvalue) %>%
    dplyr::mutate(rank = 1:n())
  #check group
  if (!is.null(group)){
    if(is.null(metadata(data_obj)[["cell_type_anno"]]) | !group %in%  colnames(metadata(data_obj)[["cell_type_anno"]])){
      stop("Something's wrong with the group column. To use this you may first do add_ct_anno()")
    }else{
      asso_df = asso_df %>% dplyr::left_join(metadata(data_obj)[["cell_type_anno"]], by="cell_type")
    }
  }
