...
#' @export
#' 
print_exp_tbl = function(data_obj, group, table_type, main_table_path, gene_id_from, aux_table_path = NULL, meta_data = NULL, assay_name = NULL, filter_rare_ct = TRUE, filter_thres = 20) {
  #check input type
  ...
  if(table_type=="MAGMA"){
    main_tbl = sweep_sparse(mean_mat, margin=2, stats = colSums(mean_mat),fun="/") %>%
      ...
      dplyrLLslice_max(specificity, prop=0.1) %>% 
    ...
    main_tbl = rbind(mean_mat, colMeans(mean_mat)) %>%
