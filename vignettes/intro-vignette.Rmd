...
#plot for the two
f3 = plot_asso(facs.sce, asso_model = "linear",trait="T2D_2", asso_model = "linear" ,group="tissue", plot_top_option = "rank<=15") + ggsci::scale_fill_npg() + ggtitle("Associated cell types with T2D") + theme(axis.text.x=element_text(size=5)) 
f4 = plot_asso(facs.sce, asso_model = "linear",trait="MDD", asso_model = "linear",group="tissue", plot_top_option = "rank<=15") + ggsci::scale_fill_npg() + ggtitle("Associated cell types with MDD") + theme(axis.text.x=element_text(size=5)) 
f3/f4
...
seismicGWAS is also able to extract the most influential genesfor a significant trait-cell type association pair.
```{r}
