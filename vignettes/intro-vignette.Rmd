...
---
This vignette introduces how to use the `seismicGWAS` (the name needs updating) package to discover associated cell types with disease from a single-cell RNA-seq data set. `seismicGWAS` will interact with the `SingleCellExperiment` object. Here is the vignette of the recommended workflow. The required data are:
...
#load processed biomaRt mmu and hsa gene mappindata("mmu_hsa_mapping")  
...
t2d_zscore = load_zscore("/grain/ql29/ad-cell-enrich/data/gwas/Tabula_muris_gs/zscore/T2D_1.genes.out")
mdd_zscore = load_zscore("/grain/ql29/ad-cell-enrich/data/gwas/Tabula_muris_gs/zscore/MDD.genes.out")
...
#plot
f1 = plot_asso(facs.sce, trait="T2D_1",plot_top_option = "rank<=15") + ggtitle("Associated cell types with T2D") + theme(axis.text.x=element_text(size=5)) 
...
#plot for the two
f3 = plot_asso(facs.sce, trait="T2D_1", group="tissue", plot_top_option = "rank<=15") + ggsci::scale_fill_npg() + ggtitle("Associated cell types with T2D") + theme(axis.text.x=element_text(size=5)) 
...
#final plot
f5 = plot_asso(facs.sce, trait="T2D_1",group="tissue", plot_top_option = "rank<=15") + ggsci::scale_fill_npg() + ggtitle("Associated cell types with T2D") + theme(axis.text.x=element_text(size=5)) 
