...
facs.sce = ct_asso(facs.sce, ra_zscore, gene_filter_setting = "det_cell_num>=10& ave_exp_ct > 0.1& max_exp_ct>0.1")
```
...
#plot
f1 = plot_asso(facs.sce, trait="T2D_2",plot_top_option = "rank<=15") + ggtitle("Associated cell types with T2D") + theme(axis.text.x=element_text(size=5)) 
...
To print out tables for MAGMA-specificity or FUMA analysis and then use MAGMA for the analysis, firstly the CPM/logCPM assays have to be obtained and then the output tables are created. Note that for the original [MAGMA-specificity](https://www.nature.com/articles/s41588-020-0610-9) and [FUMA](https://www.nature.com/articles/s41467-017-01261-5) papers, CPM and logCPM (or logcounts) assays are used respecitvely.
```{r}
#FUMA uses logcpm assay and MAGMA uses cpm assay
assay(facs.sce,"cpm") = scuttle::calculateCPM(facs.sce)
#calculate mean expression
facs.sce = cal_stat(data_obj = facs.sce, meta_data = as.data.frame(colData(facs.sce)), group = "cluster_name",assay_name = "cpm", mean_only = TRUE)
#map mmu genes to hsa genes
facs.sce = trans_mmu_to_hsa_stat(facs.sce, gene_mapping_table=mmu_hsa_mapping, from="mmu_symbol", to="hsa_entrez")
#write.out
