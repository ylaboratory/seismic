...
`seismicGWAS`, use `devtools::install(path_to_seismic_folder)`.
...
Next, to aid in result interpretation we make use of the `biomaRt` package to
...
# Section 2: running _seismic_ on Tabula Muris Senis
...
Explore the SCE object to find the relevant cell type column.
```{r}
colData(sce_panc)
```
